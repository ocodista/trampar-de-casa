...
2. Execute o comando `make setup-env` para criar os arquivos `.env`.  
  2.! Se o sistema não for GNU, crie uma cópia do arquivo `.env.example` chamada `.env`
  e do arquivo `supabase/.env.example` em `supabase/.env`. Então preencha a propriedade
  `INCLUDE_PATH` no `.env` raiz, com o resultado do comando `pwd`.   
